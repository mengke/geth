...
object ByteString {
  ...
  private[geth] object ByteString1C extends Companion {
  ...
  final class ByteString1C private (private val bytes: Array[Byte]) extends CompactByteString {
    ...
    /** INTERNAL API */
    private[geth] def toByteString1: ByteString1 = ByteString1(bytes, 0, bytes.length)

    /** INTERNAL API */
    private[geth] def byteStringCompanion = ByteString1C
    ...
    private[geth] override def writeToOutputStream(os: ObjectOutputStream): Unit =
    ...
    private[geth] def appendToBuilder(buffer: ByteStringBuilder) = {
      buffer.putByteArrayUnsafe(bytes)
    }
  }

  /** INTERNAL API: ByteString backed by exactly one array, with start / end markers */
  private[geth] object ByteString1 extends Companion {
  ...
  final class ByteString1 private (private val bytes: Array[Byte], private val startIndex: Int, val length: Int) extends ByteString with Serializable {
    ...
    private[geth] def writeToOutputStream(os: ObjectOutputStream): Unit = {
      os.writeInt(length)
    ...
    private[geth] def byteStringCompanion = ByteString1

    override def dropRight(n: Int): ByteString =
      dropRight1(n)

    /** INTERNAL API */
    private[geth] def dropRight1(n: Int): ByteString1 =
      if (n <= 0) this
    ...
    /** INTERNAL API */
    private[geth] def drop1(n: Int): ByteString1 = {
  ...
  private[geth] object ByteStrings extends Companion {
  ...
  /**
    ...
    */
  final class ByteStrings private (private[geth] val bytestrings: Vector[ByteString1], val length: Int) extends ByteString with Serializable {
    ...
    private[geth] def byteStringCompanion = ByteStrings
    ...
    private[geth] def take0(n: Int): ByteString = {
  ...
  private[geth] sealed trait Companion {
...
sealed abstract class ByteString extends IndexedSeq[Byte] with IndexedSeqOptimized[Byte, ByteString] {
  ...
  private[geth] def byteStringCompanion: ByteString.Companion
...
final class ByteStringBuilder extends mutable.Builder[Byte, ByteString] { builder ⇒
  ...
  override def +=(elem: Byte): ByteStringBuilder.this.type = {
  ...
  private[geth] def putByteArrayUnsafe(xs: Array[Byte]): this.type = {
    clearTemp()
