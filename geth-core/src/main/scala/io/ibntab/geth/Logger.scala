...
class Logger private (val logger: Slf4jLogger, isEnabled: => Boolean) extends LoggerLike {
  ...
  /**
    ...
    */
  def forMode(mode: Profile*): Logger = {
    modeLoggerCache.getOrElseUpdate(mode, new Logger(logger, Logger.applicationProfile.forall(mode.contains)))
  }

  private[this] val modeLoggerCache: mutable.Map[Seq[Profile], Logger] =
    new ConcurrentHashMap[Seq[Profile], Logger]().asScala
}
...
object Logger extends Logger(LoggerFactory.getLogger("application")) {
  ...
  private[this] var _profile: Option[Profile] = None
  ...
  /**
    ...
    */
  def applicationProfile: Option[Profile] = _profile

  /**
    * Set the global application mode used for logging. Used when the Play application starts.
    */
  def setApplicationMode(profile: Profile): Unit = {
    val appsRunning = _appsRunning.incrementAndGet()
    applicationProfile foreach { currentProfile =>
      if (currentProfile != profile) {
        log.warn(s"Setting logging profile to $profile when it was previously set to $currentProfile")
  ...
  def unsetApplicationMode(): Unit = {
    ...
    if (appsRunning == 0) {
      ...
      log.warn("Cannot unset application profile because none was previously set")
      _profile = None
